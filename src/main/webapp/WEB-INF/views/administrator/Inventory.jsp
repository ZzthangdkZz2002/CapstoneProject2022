                        ...
                        <form method="get" action="/admin/inventory/search">
                            <div class="form-check my-3">
                                ...
                                <label class="form-check-label d-flex justify-content-between line-under" for="thisMonth">
                                    <span>Theo ngày tháng</span>
                                    <i class="fa-solid fa-sort pt-1 btn-time"></i>
                                </label>
                            </div>
                            <div class="form-check my-3 ">
                                <input class="form-check-input" type="radio" name="inventoryDate" id="otherChoices">
...
<script>
    const otherchoice = document.getElementById('otherChoices')
    const thismonth = document.getElementById('thisMonth')
    otherchoice.addEventListener('change',valueChange)
    thismonth.addEventListener('change',valueChange)
    function valueChange(event) {
        if(otherchoice.checked){
            document.getElementById('btnInventorySearch').style.display = "block";
        }else{
            document.getElementById('btnInventorySearch').style.display = "none";
        }
    }
</script>
...
<script>
    ...
    //     var date = $('#datetimerange-input1').val();
    //
    //     $.ajax(
    //         {
    //             type : "GET",
    //             url : "/admin/inventory/search?datedetail="+date,
